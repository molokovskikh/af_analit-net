...
namespace Analit.Net.Componets
{
	...
	public class MenuList : ViewComponent
	{
		public override void Render()
		{
			...
			foreach (var menuField in menu)
			{
				...
				var applicationPath = menuField.Link.Contains("http") ? string.Empty : EngineContext.ApplicationPath + "/";
				blockMenu += string.Format("<li class=\"main {2}\">" +
						   "<a href=\"{0}\" id=\"{3}\" class=\"menu-item one-line {2}\">{1}</a><br />", applicationPath + menuField.Link,
				...
				//if (menuField.subMenu.Count != 0 || mitemFirst)
				{
					...
					{
						var subAppPath = subField.Link.Contains("http") ? string.Empty : EngineContext.ApplicationPath + "/";
						blockMenu += string.Format("<li><a href=\"{0}\" style=\"display:{2}\" class=\"submenu-item\">{1}</a></li>", subAppPath + subField.Link, subField.Name, subMenDis);
					}
	...
	public class EditMenuList : ViewComponent
	{
		public string GenerateSelectList(string id, string selectedLink)
		{
			...
			foreach (var item in MenuField.Queryable.Where(el => EditorController.SpecialLinks.Contains(el.Link)))
			{
				...
				select += string.Format("<option name=\"{3}\" value=\"{0}\" {2}>{1}</option>", item.Link, item.Name.Substring(0, item.Name.Length >= 25 ? 25 : item.Name.Length), selected, nameIndex);
				nameIndex++;
			}
			foreach (var item in EditorController.SpecialLinks.Where(t => !MenuField.FindAll().Select(g => g.Link).Contains(t)))
			{
				select += string.Format("<option value=\"{0}\" >{1}</option>", item, item.Substring(0, item.Length >= 25 ? 25 : item.Length ));
			}
